//var surf_tempStorage = surface_create(room_w,room_h);
...
surface_set_target(surf_tempVelocity);
	shader_set(shd_velocityAdvection);
		//shader_set_uniform_f(adSize,size/room_w,size/room_h);
		//shader_set_uniform_f(adScale,1.0/scale);
		shader_set_uniform_f(texSize,1/room_w,1/room_h);
		shader_set_uniform_f(time,stepTime);
		shader_set_uniform_f(useMac, true);
		shader_set_uniform_f(adDiss,1.0,velocityDissipation);
		shader_set_uniform_f(macW, 0.5 * 1.0);
...
surface_set_target(surf_tempDensity);
	shader_set(shd_densityAdvection);
		//shader_set_uniform_f(adSize,size/room_w,size/room_h);
		//shader_set_uniform_f(adScale,1.0/scale);
		//shader_set_uniform_f(time,stepTime);
		shader_set_uniform_f(densDiss,materialDissipation);
		shader_set_uniform_f(densTexSize,1/room_w,1/room_h);
		shader_set_uniform_f(densTime,stepTime);
		//shader_set_uniform_f(useMac, true);
		//shader_set_uniform_f(adDiss,1.0,materialDissipation);
		//shader_set_uniform_f(macW, 0.5 * 1.0);
		texture_set_stage(shader_get_sampler_index(shd_densityAdvection, "velocity_field"), surface_get_texture(surf_velocity));
...
surface_set_target(surf_tempDiffusion);
	shader_set(shd_addVectors);
		shader_set_uniform_f(ascale,1.0);
		shader_set_uniform_f(bscale,0.0);
		texture_set_stage(shader_get_sampler_index(shd_addVectors,"a_field"),surface_get_texture(surf_velocity));
		texture_set_stage(shader_get_sampler_index(shd_addVectors,"b_field"),surface_get_texture(surf_diffusion));
		draw_surface(surf_diffusion,0,0);
	shader_reset();
surface_reset_target();
	scr_swapFields(surf_tempStorage,surf_tempDiffusion,surf_diffusion);

//Estimate diffusion field with jacobi
for(var i=0;i<jacobiIterations;i++) {
	surface_set_target(surf_tempDiffusion);
		shader_set(shd_jacobi);
			shader_set_uniform_f(diffAlph,defaultViscAlpha);
			shader_set_uniform_f(diffBeta,defaultViscBeta);
			texture_set_stage(shader_get_sampler_index(shd_jacobi,"x_field"),surface_get_texture(surf_diffusion));
			texture_set_stage(shader_get_sampler_index(shd_jacobi,"b_field"),surface_get_texture(surf_diffusion));
	          draw_surface(surf_diffusion, 0, 0);
		shader_reset();
	surface_reset_target();
		scr_swapFields(surf_tempStorage,surf_tempDiffusion,surf_diffusion);
...
surface_set_target(surf_tempVelocity);
	shader_set(shd_addVectors);
		shader_set_uniform_f(ascale,0.0);
...
surface_set_target(surf_tempTemperature)
	shader_set(shd_buoyancy);
		...
		shader_set_uniform_f(buoyBeta,1.0);
			...
			texture_set_stage(shader_get_sampler_index(shd_buoyancy,"density_field"),surface_get_texture(surf_density));
			texture_set_stage(shader_get_sampler_index(shd_buoyancy,"temperature_field"),surface_get_texture(surf_temperature));
				draw_surface(surf_temperature,0,0);
			shader_reset();
surface_reset_target();
	scr_swapFields(surf_tempStorage,surf_tempTemperature,surf_temperature);
...
surface_set_target(surf_tempVorticity)
	...
	shader_reset();
surface_reset_target();
	 scr_swapFields(surf_tempStorage,surf_tempVorticity,surf_vorticity);
...
surface_set_target(surf_tempDivergence);
	...
	shader_set_uniform_f(divScale,0.5);
    ...
    shader_reset();
surface_reset_target();
	scr_swapFields(surf_tempStorage,surf_tempDivergence,surf_divergence);
...
for(var i=0;i<jacobiIterations;i++) {
	surface_set_target(surf_tempPressure);
		shader_set(shd_jacobi);
			...
			texture_set_stage(shader_get_sampler_index(shd_jacobi,"divergence_field"),surface_get_texture(surf_divergence));
				draw_surface(surf_pressure, 0, 0);
		shader_reset();		
	surface_reset_target();
		scr_swapFields(surf_tempStorage,surf_tempPressure,surf_pressure);
}

//Subtract from the gradient		
surface_set_target(surf_tempVelocity);
	shader_set(shd_subtractGradient);
		shader_set_uniform_f(gradScale,0.5);
