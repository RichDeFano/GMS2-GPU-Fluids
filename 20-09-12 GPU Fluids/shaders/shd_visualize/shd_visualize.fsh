...
void main() {
	...
	float scalar = getScalarFromTextureU(texture2D(gm_BaseTexture, v_vTexcoord));
	vec2 vector = getVectorFromTexture(texture2D(gm_BaseTexture,v_vTexcoord));
	
	if (isVector == false)
   { gl_FragColor = v_vColour * vec4(vec3(scalar),0.5);}
   else
   {gl_FragColor = v_vColour * vec4(vector,0.0,0.5);}
