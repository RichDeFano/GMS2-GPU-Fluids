...
void main() {
    ...
    float left = getScalarFromTextureUnsigned(texture2D(x_field, coords - vec2(1, 0)));
    float right = getScalarFromTextureUnsigned(texture2D(x_field, coords + vec2(1, 0)));
    float bottom = getScalarFromTextureUnsigned(texture2D(x_field, coords - vec2(0, 1)));
    float top = getScalarFromTextureUnsigned(texture2D(x_field, coords + vec2(0, 1)));

	float div = getScalarFromTextureSigned(texture2D(b_field,coords));
	float mag = div/10.0;
    float temp = ((left + right + bottom + top + alpha * div) / rBeta);
	gl_FragColor = setScalarToTextureUnsigned(temp,1.0*mag);
