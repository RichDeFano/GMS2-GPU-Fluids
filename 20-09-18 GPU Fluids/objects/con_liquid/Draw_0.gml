...
if ((surface_exists(surf_tempDensity)) && (surface_exists(surf_density))) && ((surface_exists(surf_tempVelocity)) && (surface_exists(surf_velocity)))
{
	...
	surface_set_target(surf_tempVelocity);
		shader_set(shd_gaussVelocity);
			...
			shader_set_uniform_f(gaussRad,16.0);
	...
	/*
		...
		shader_set_uniform_f(isSig,false);
		texture_set_stage(shader_get_sampler_index(shd_fieldVisualization,"visual_field"),surface_get_texture(surf_tempDensity));
			draw_surface(surf_density,0,0);
	shader_reset();
		draw_text(0,0,"Density");
		*/
...
//gpu_set_blendenable(true);
	...
	surface_set_target(surf_tempVelocity);
		shader_set(shd_gaussVelocity);
			shader_set_uniform_f(prevPt,mouseprev_x,mouseprev_y);
			shader_set_uniform_f(gaussPt,mouse_x,mouse_y);
			shader_set_uniform_f(gaussRad,16.0*scale);
			shader_set_uniform_f(gaussTime,stepTime);
				draw_surface(surf_velocity,0,0);
		shader_reset();
	surface_reset_target();
...
/*
	surface_set_target(surf_tempVelocity);
		shader_set(shd_splat);
			...
			shader_set_uniform_f(vr,64.0);
			shader_set_uniform_f(vfC,255.0,255.0,0.0);
			texture_set_stage(shader_get_sampler_index(shd_splat,"vector_field"),surface_get_texture(surf_velocity));
				draw_surface(surf_velocity,0,0);
				//draw_sprite_ext(spr_grad,0,mouse_x,mouse_y,1,1,0,c_green,1.0);
		shader_reset();
	surface_reset_target();
		scr_swapFields(tempStorage,surf_tempVelocity,surf_velocity);
*/
...
//gpu_set_blendmode(bm_normal);
	...
	surface_set_target(surf_tempVelocity);
		shader_set(shd_gaussVelocity);
			...
			shader_set_uniform_f(gaussPt, mouse_x,mouse_y);
			shader_set_uniform_f(gaussRad, 16.0);
			texture_set_stage(shader_get_sampler_index(shd_gaussVelocity,"vector_field"),surface_get_texture(surf_velocity));
				draw_surface(surf_velocity,0,0);
				draw_sprite_ext(spr_grad,0,mouse_x,mouse_y,1,1,0,c_yellow,1.0);
		shader_reset();
	surface_reset_target();
	surface_set_target(surf_tempDensity);
		draw_surface(surf_density,0,0);
		draw_sprite_ext(spr_grad,0,mouse_x,mouse_y,scale,scale,0,c_white,0.5);
	surface_reset_target();
	//gpu_set_blendmode(bm_normal);
	...
	surface_set_target(surf_tempVelocity);
		draw_surface(surf_velocity,0,0);
		draw_sprite_ext(spr_grad,0,mouse_x,mouse_y,scale,scale,0,c_yellow,0.5);
	surface_reset_target();
	*/
/*	
//Fluid
