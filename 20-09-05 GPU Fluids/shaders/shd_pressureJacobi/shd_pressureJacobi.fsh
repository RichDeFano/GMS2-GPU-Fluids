...
void main() {
	...
	*/
    ...
    float left = unpack_ufloat_16(texture2D(pressure_field, coords - vec2(-1.0, 0.0)).xy) * 10.0;//texelFetchOffset(pressure, coords, 0, ivec2(-1, 0));
    float right = unpack_ufloat_16(texture2D(pressure_field, coords + vec2(1.0, 0.0)).xy) * 10.0;
    float bottom = unpack_ufloat_16(texture2D(pressure_field, coords + vec2(0.0, -1.0)).xy) * 10.0;
    float top = unpack_ufloat_16(texture2D(pressure_field, coords + vec2(0.0, 1.0)).xy) * 10.0;

    // Get the pressure at our current cell
    float center = unpack_ufloat_16(texture2D(divergence_field,coords).xy);//texelFetch(divergence, coords, 0);

    // Calculate the Jacobi approximation for the Poisson-pressure equation
   vec2 temp = pack_ufloat_16(((left + right + bottom + top + alpha * center) * rBeta)/10.0);
