...
void main()
{
	...
	if (isVector)
	{
		...
		result = vec4(1.0,1.0,0.0,0.5) * texture2D( visual_field, v_vTexcoord );
	}
	else
	{
		result = texture2D( visual_field, v_vTexcoord );
		
		if (isSigned)
		{
		float scalar = getScalarFromTextureSigned(texture2D(visual_field,coords));
		float mag = scalar/10.0;
		result = vec4(1.0,0.0,0.0,0.5) * texture2D( visual_field, v_vTexcoord );
		}
		else
		{
		//	float scalar = getScalarFromTextureUnsigned(texture2D(visual_field,coords));
		//float mag = clamp(scalar/10.0,0.0,1.0);
		result = texture2D( visual_field, v_vTexcoord );//setScalarToTextureSigned(scalar,mag);	
		//result.a = mag;
		}
