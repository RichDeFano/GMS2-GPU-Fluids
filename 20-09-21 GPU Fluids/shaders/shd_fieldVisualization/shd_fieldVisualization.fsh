...
void main()
{
	...
	if (isVector)
	{
		...
		result = vec4(255.0,255.0,0.0,1.0) * texture2D( gm_BaseTexture, v_vTexcoord );
	}
	else
	{
		if (isSigned)
		...
		result = vec4(0.0,0.0,255.0,mag) * texture2D( gm_BaseTexture, v_vTexcoord );
		}
		else
		{
		
		float scalar = texture2D(visual_field,coords).x;
		float mag = scalar/255.0;
		result = vec4(255.0,0.0,0.0,1.0) * texture2D( gm_BaseTexture, v_vTexcoord );//setScalarToTextureSigned(scalar,mag);		
		}
	}
