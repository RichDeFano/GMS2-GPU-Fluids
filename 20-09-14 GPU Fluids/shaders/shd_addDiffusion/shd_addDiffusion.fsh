...
void main()
{
    ...
    vec4 color = scaleA * texture2D(vector_field,coords) + scaleB * texture2D(scalar_field, coords);//((scaleA * getVectorFromTexture(texture2D(vector_field, coords))) * (scaleB * getScalarFromTextureUnsigned(texture2D(scalar_field, coords))));
