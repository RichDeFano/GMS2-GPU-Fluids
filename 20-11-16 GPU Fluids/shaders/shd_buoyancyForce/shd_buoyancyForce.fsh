...
void main()
{
	...
	float f_buoy = getScalarFromTextureUnsigned(texture2D(temperature_field,coords));//( (-alpha * getScalarFromTextureUnsigned(texture2D(density_field,coords))) + ( (beta) * (getScalarFromTextureUnsigned(texture2D(temperature_field,coords)) - 0.0)));
