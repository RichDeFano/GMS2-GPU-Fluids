...
void main()
{
	...
	result = texture2D(gm_BaseTexture, v_vTexcoord );

/*
	if (isVector)
	{
		vec2 scalar = getVectorFromTexture(texture2D(gm_BaseTexture,coords));
		float mag = scalar.x/10.0;
		result = vec4(1.0,1.0,0.0,0.5) * texture2D( gm_BaseTexture, v_vTexcoord );
	}
	else
	{
		//result = texture2D( gm_BaseTexture, v_vTexcoord );
		...
		if (isSigned)
		{
		float scalar = getScalarFromTextureSigned(texture2D(gm_BaseTexture,coords));
		float mag = scalar/10.0;
		result = vec4(1.0,0.0,0.0,0.5) * texture2D( gm_BaseTexture, v_vTexcoord );
		}
		else
		{
		//	float scalar = getScalarFromTextureUnsigned(texture2D(visual_field,coords));
		//float mag = clamp(scalar/10.0,0.0,1.0);
		result = texture2D(visual_field, coords );//setScalarToTextureSigned(scalar,mag);	
		//result.a = mag;
		}
		
	}

    gl_FragColor = result;/// * texture2D( gm_BaseTexture, v_vTexcoord );
	*/
}
