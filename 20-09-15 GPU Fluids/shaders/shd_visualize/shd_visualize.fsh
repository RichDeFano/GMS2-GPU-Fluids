...
vec4 setVectorToTexture(vec2 vector){
	...
	vec4 newVTexture = vec4(normalizedVector,0.0,0.5);
	return newVTexture;
}
...
vec4 setScalarToTextureUnsigned(float scalar){
	...
	vec4 newSTexture = vec4(normalizedScalar,0.0,0.0,0.5);
	return newSTexture;
}
...
void main() {
	...
	if (isVector == true)
	{
		...
		gl_FragColor = setVectorToTexture(vel);
	}
	else
	{
		float scalar;
		if (isNegative == true)
			...
			gl_FragColor = setScalarToTextureSigned(scalar);
			}
		else
			{
			scalar = getScalarFromTextureUnsigned(texture2D(vector_field,coords));
			gl_FragColor = setScalarToTextureUnsigned(scalar);
			}
	}
}
	/*
