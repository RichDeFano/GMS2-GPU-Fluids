...
if (mousePressed == true){
	...
	surface_set_target(surf_tempDensity);
		shader_set(shd_gaussianSplat)
			...
			shader_set_uniform_f(r,42.0);
			shader_set_uniform_f(fC,138.0,43.0,226.0);
			texture_set_stage(shader_get_sampler_index(shd_gaussianSplat,"vector_field"),surface_get_texture(surf_density));
	...
	surface_set_target(surf_tempVelocity);
		shader_set(shd_gaussianSplat);
			...
			shader_set_uniform_f(vr,42.0);
			shader_set_uniform_f(vfC,124.0,252.0,0.0);
			texture_set_stage(shader_get_sampler_index(shd_gaussianSplat,"vector_field"),surface_get_texture(surf_velocity));
