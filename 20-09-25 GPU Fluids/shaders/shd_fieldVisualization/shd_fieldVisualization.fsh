...
void main()
{
	...
	if (isVector)
	{
		...
		result = vec4(1.0,1.0,0.0,1.0) * texture2D( gm_BaseTexture, v_vTexcoord );
	}
	else
	{
		if (isSigned)
		...
		result = vec4(0.0,0.0,1.0,1.0) * texture2D( gm_BaseTexture, v_vTexcoord );
		}
		else
		{
		...
		//float mag = scalar/10.0;
		result = vec4(1.0,0.0,0.0,1.0) * texture2D( gm_BaseTexture, v_vTexcoord );//setScalarToTextureSigned(scalar,mag);		
		}
	}
